...
test.run(function()
    ...
    local base_equates = test.load_equates(binary_directory .. "/invaders.equ")
    local symbols = test.load_symbols(binary_directory .. "/invaders-avo.sym")
    local inv_test_signature = test.required_equate(base_equates, "inv_test_signature")
    ...
    test.assert_eq(read_u8(inv_test_signature), 0x05, "AVO RAM signature byte 0")
    test.assert_eq(read_u8(inv_test_signature + 1), 0x0a, "AVO RAM signature byte 1")
    test.assert_eq(read_u8(inv_test_signature + 2), 0x0f, "AVO RAM signature byte 2")
